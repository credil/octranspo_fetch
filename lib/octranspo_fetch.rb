...
class OCTranspo
  ...
  def get_route_summary_for_stop(stop, options = {})
    ...
    result = {
      stop: get_value(xresult, "StopNo"),
      stop_description: get_value(xresult, "StopDescription"),
      routes: [],
    }

    xresult.xpath("Routes/Route").each do |route|
      result[:routes].push({
        route: get_value(route, "RouteNo"),
        direction_id: get_value(route, "DirectionID"),
        direction: get_value(route, "Direction"),
        heading: get_value(route, "RouteHeading"),
      })
    end
  ...
  def get_next_trips_for_stop(stop, route_no = nil, options = {})
    ...
    if route_no.nil?
      parts = {
        ...
        :route_xpath => "Routes/Route",
        :route_error => "Error for route",
        :route_label => "RouteHeading",
      }
    else
      parts = {
        ...
        :alt_resource => nil,
        :stop_description => "StopLabel",
        :route_xpath => "Route/RouteDirection",
    ...
    xresult.xpath(parts[:route_xpath], OCT_NS).each do |route|
      ...
      route.xpath("Trips/Trip", OCT_NS).each do |trip|
        route_obj[:trips].push({
          ...
          adjusted_schedule_time: get_value(trip, "AdjustedScheduleTime").to_i, # Adjusted schedule time in minutes
          adjustment_age: get_value(trip, "AdjustmentAge").to_f, # Time since schedule was adjusted in minutes
          last_trip: (get_value(trip, "LastTripOfSchedule") == "true"),
          bus_type: get_value(trip, "BusType"),
          gps_speed: get_value(trip, "GPSSpeed").to_f,
          latitude: get_value(trip, "Latitude").to_f,
          longitude: get_value(trip, "Longitude").to_f,
        })
      end
  ...
  BASE_URL = "https://api.octranspo1.com/v2.0"
  ...
  def fetch(resource, params, alt_resource = nil)
    ...
    response = RestClient.post("#{BASE_URL}/#{resource}",
                               "appID=#{@app_id}&apiKey=#{@app_key}&#{params}&format='XML'")
    if alt_resource.nil?
